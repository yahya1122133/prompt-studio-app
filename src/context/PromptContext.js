...
export const PromptProvider = ({ children }) => {
    ...
    const handleGenerateResponse = useCallback(async (finalPrompt, showStatus) => {
        ...
        // Local development fallback - use mock unless explicitly disabled
        ...
        // Allow bypassing local mock by adding ?api=real to URL or using test provider
        const useRealAPI = new URLSearchParams(window.location.search).get('api') === 'real' || 
                          state.provider === 'test';
        
        if (isLocalDev && !useRealAPI) {
            console.log('🧪 Using local development mock (bypassing API)', { hostname: window.location.hostname, port: window.location.port });
            try {
                // Simulate API delay
                ...
                const mockResponses = [
                    `Great prompt! Here's a creative response for: "${finalPrompt.substring(0, 40)}${finalPrompt.length > 40 ? '...' : ''}"\n\nThis is a local development mock response. The actual AI providers will generate different content when deployed.`,
                    `Mock AI Response:\n\nYour prompt "${finalPrompt.substring(0, 30)}${finalPrompt.length > 30 ? '...' : ''}" has been processed!\n\nIn production, this would be generated by ${state.provider === 'auto' ? 'Gemini or DeepSeek' : state.provider}.`,
                    `✨ Development Response ✨\n\nPrompt processed: "${finalPrompt.substring(0, 35)}${finalPrompt.length > 35 ? '...' : ''}"\n\nThis mock response helps you test the UI locally without API calls.`,
                    `🧪 Test Response\n\nYour request: "${finalPrompt.substring(0, 30)}${finalPrompt.length > 30 ? '...' : ''}"\n\nDeploy to Netlify to use real AI models!`
                ];
                
                const mockResponse = {
                    provider: state.provider === 'test' ? 'test' : `${state.provider} (local)`,
                    text: mockResponses[Math.floor(Math.random() * mockResponses.length)],
                    metrics: { 
                        duration: Math.round(1500 + Math.random() * 2000), 
                ...
                stopProgressSimulation();
                dispatch({ type: 'SET_API_RESPONSE', payload: mockResponse });
                dispatch({ type: 'ADD_TO_HISTORY', payload: mockResponse });
                showStatus(`Success! (local mock)`, "success");
                dispatch({ type: 'LEARN_FROM_SUCCESS', payload: finalPrompt });
                dispatch({ type: 'SET_LOADING', payload: false });
                return;
            } catch (error) {
                if (error.name === 'AbortError') return;
                console.error('Local mock failed:', error);
