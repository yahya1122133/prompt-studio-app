...
export default function App() {
  ...
  // Prompt library state, now managed with localStorage
  ...
  useEffect(() => {
    try {
      ...
      if (promptsFromStorage) {
        setSavedPrompts(JSON.parse(promptsFromStorage));
      }
    } catch (error) {
  ...
  const handleSavePrompt = () => {
    ...
    if (selectedPromptId) {
        ...
        newPrompts = savedPrompts.map(p => 
            p.id === selectedPromptId ? { ...p, name: cleanName, template: promptTemplate } : p
        );
    } else {
        // Add new prompt
        const newPrompt = {
        ...
        newPrompts = [...savedPrompts, newPrompt];
  ...
  const cancelDelete = () => setPromptToDelete(null);
          ...
          <div className="flex flex-col gap-6">
            <Card title="Prompt Template" icon={<Wand2 className="text-indigo-400" />}>
              <p className="text-sm text-gray-400 mb-3">
                1. Edit your template. Use double curly braces for variables (e.g. topic).
              </p>
              <textarea
